...
description: 'This document explains how Lago is integrated into the Stratala for subscription billing and payment management.
'
navigation:
  title: 'Lago Billing Integration'
  order: 3
...
[Lago](https://www.getlago.com/) is an **open-source** billing platform designed
for modern SaaS applications. It provides:
...
```typescript
await BillingService.linkPayPalCustomer(user.id, paypalCustomerId);
```
