...
This guide walks you through deploying the Stratala to production with AdonisJS as the API gateway.

## Architecture Overview

```
Internet → Nginx (443/80) → AdonisJS (3333) → {Payload, Lago, PostgreSQL}
                          ↓
                        Nuxt (3000)
```
...
- **Databases**: `adonis_db`, `payload_db`, `lago_db`
- **Backups**: Configure automated backups

```bash
# Manual backup
docker-compose exec postgres pg_dump -U postgres adonis_db > backup.sql
docker-compose exec postgres pg_dump -U postgres payload_db > payload_backup.sql
...
docker-compose exec -T postgres psql -U postgres payload_db < payload_backup.sql
...
- **Port**: 3001 (internal)
- **Access**: Via AdonisJS proxy at `/api/cms/*` (Local API) or REST API at `/api/*`
...
- **Endpoints**:
  ...
  - `/api/cms/*` - Payload proxy (Local API)
...
docker-compose exec postgres pg_dump -U postgres payload_db > backups/payload_$DATE.sql
docker-compose exec postgres pg_dump -U postgres lago_db > backups/lago_$DATE.sql
```

### File Backups

- Payload uploads: `apps/studio/media/`
- Application data
- SSL certificates
...
- Shared file storage for Payload uploads (S3, NFS)
