---
title: 'Architecture'
description: 'Production-ready architecture overview for the Stratala'
navigation:
  title: 'Architecture'
  order: 1
---
...
This document describes the production-ready architecture for the Stratala, with
AdonisJS serving as the central API gateway.
...
**Purpose**: Entry point for all traffic, handles SSL termination, routing, and
security.
...
**Purpose**: Single point of entry for all API requests, handles authentication,
and proxies to internal services.
...
- **API Gateway**
  ...
  - Access Payload CMS via Local API (direct database access)
...
GET    /api/cms/*                  - Payload Local API (read)
POST   /api/cms/*                  - Payload Local API (create)
PATCH  /api/cms/*                  - Payload Local API (update)
DELETE /api/cms/*                  - Payload Local API (delete)
...
GET    /api/home                   - Home page content (trending + editor's picks)
GET    /api/feed                   - Personalized feed (requires auth)
GET    /api/feed/spaces            - Get followed spaces (requires auth)
POST   /api/feed/spaces/:id/follow - Follow a space (requires auth)
DELETE /api/feed/spaces/:id/follow - Unfollow a space (requires auth)
GET    /api/explore                - Explore content with filters
GET    /api/explore/spaces         - Creator directory
GET    /api/trending               - Trending content
POST   /api/content/:type/:id/view - Track content view
POST   /api/content/:type/:id/like - Like content (requires auth)
DELETE /api/content/:type/:id/like - Unlike content (requires auth)
GET    /api/content/:type/:id/like-status - Get like status (requires auth)
...
**Purpose**: Server-side rendered frontend application with user and admin
interfaces.
...
- `adonis_db` - AdonisJS application data
  ...
  - users
  - sessions
  - user_follows (content discovery)
  - content_views (engagement tracking)
  - content_likes (engagement tracking)
  - application-specific tables

- `payload_db` - Payload CMS data

  - collections (spaces, posts, pages, etc.)
...
- API for content delivery (REST/GraphQL)
- Multi-tenant content ownership
- User authentication (synced from Better Auth/AdonisJS)

**Access Pattern**:

```
Frontend → Payload REST API (http://localhost:3002/api/*)
Backend → Payload Local API (direct database access)
```

**Features**:

- Dynamic content types (collections)
...
- Admin panel at http://localhost:3002/admin
...
1. User requests blog posts
   └─> Nuxt page (dashboard/index.vue)
       └─> GET /api/posts → Nuxt Server API
           └─> GET http://localhost:3002/api/posts (Payload REST API)
           └─> Return data to Nuxt
...
2. Admin creates new post
   └─> Nuxt admin panel
       └─> POST /api/cms/posts → AdonisJS
           └─> Verify admin role
           └─> Payload Local API (direct database access)
...
- Redis: `PING` command
- Payload CMS: HTTP health endpoint at http://localhost:3002/api/health
...
- ✅ Payload CMS (stateless, requires shared storage for uploads)
...
- Payload CMS uploads folder (media files)
...
2. Restore Payload CMS uploads (media files)
3. Restart services
4. Verify data integrity
5. Monitor for issues

RTO (Recovery Time Objective): < 1 hour RPO (Recovery Point Objective): < 24
hours
...
- **[API Architecture](./2.api-architecture.md)** - Hybrid API architecture
  using Nuxt Server API and AdonisJS Gateway
- **[Spaces & Profile System](./3.spaces-profile-system.md)** - Multi-tenant
  spaces and user profile system
- **[Package Structure](./4.package-structure.md)** - Monorepo organization and
  package management
- **[Pinia State Management](./5.pinia-state-management.md)** - Frontend state
  management patterns
- **[Routing System](./6.routing-system.md)** - Nuxt routing and navigation
- **[Subdomain Tenant Routing](./7.subdomain-tenant-routing.md)** -
  Subdomain-based tenant routing system
- **[Content Discovery System](./8.content-discovery-system.md)** - Home, My
  Feed, and Explore content discovery features
