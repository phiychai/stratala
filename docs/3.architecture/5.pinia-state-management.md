...
This guide covers the state management architecture using Pinia stores in the
application.

## Overview

**Pinia** is the official state management library for Vue.js and Nuxt.js. It
provides:
...
The existing `useAuth()` composable now wraps the Pinia store for backward
compatibility:
...
const canAccessPremiumFeatures = computed(() => {
  return authStore.isAuthenticated && billingStore.hasActiveSubscription;
});
```
...
For new features, always use Pinia stores directly for the best developer
experience!
