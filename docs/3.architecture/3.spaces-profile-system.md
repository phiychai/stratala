...
**Spaces are stored in Payload CMS**, not in AdonisJS. The `spaces` collection is a Payload collection that lives in the Payload database.
...
The system needs to resolve usernames (from AdonisJS) to Payload user IDs. This is handled by:
...
**Fallback:**
If user exists in AdonisJS but not in Payload, the profile page will still load with empty spaces/posts.
