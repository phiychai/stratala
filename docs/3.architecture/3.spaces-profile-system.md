...
description:
  'Substack/Medium-like content organization with user spaces and public
  profiles'
navigation:
...
The Spaces & Profile system enables a Substack/Medium-like content organization
where users can create multiple "spaces" (collections) to organize their posts.
Each user has a public profile page that displays their spaces and recent
content.
...
A **Space** is a user-owned collection for organizing posts. Think of it like a
Substack publication or Medium publication.
...
**Spaces are stored in Payload CMS**, not in AdonisJS. The `spaces` collection
is a Payload collection that lives in the Payload database.
...
- Resolves user identity (maps AdonisJS user → Payload user)
- Enforces business rules before creating in Payload
- Provides a consistent authenticated API interface
- Uses Payload Local API for direct database access with user context

**Flow:**

```
Client → AdonisJS API (validates/auth) → Payload Local API (stores data with user context)
```
...
interface Space {
  ...
  createdBy: PayloadUser; // User who owns this space (for tenant isolation)
  isDefault: boolean; // If true, this is the default space for articles
...
```
payload_users (1) ──< (many) spaces (via createdBy)
spaces (1) ──< (many) posts
payload_users (1) ──< (many) posts (via createdBy)
```
...
These endpoints are handled by AdonisJS but **store data in Payload CMS**.
...
2. Resolves Payload user ID from session
3. Queries Payload `spaces` collection
...
3. Resolves Payload user ID from session
4. Checks for duplicate slug (per user)
5. **Creates space in Payload** via `payloadService.create('spaces', ...)`
...
- `name`: Required
- Reserved slugs: `article`, `articles`, `blog`, `admin`, `dashboard`,
  `settings`, `login`, `signup`, `api`, `docs`, `explore`, `library`,
  `changelog`

**Storage:** Space is stored in Payload `spaces` collection
...
The system needs to resolve usernames (from AdonisJS) to Payload user IDs. This
is handled by:
...
3. Find Payload user by email
4. Return Payload user ID

**Fallback:** If user exists in AdonisJS but not in Payload, the profile page
will still load with empty spaces/posts.
...
Each user should have a default space (marked with `is_default: true`) for
general articles. This space typically uses the slug `article`.

**Migration:** The `migrate-spaces.ts` script can create default spaces for
existing users and assign existing posts to them.
...
The catch-all route (`[...permalink].vue`) excludes routes starting with `@` to
ensure space/profile routes take precedence:
...
The path-based routing is implemented first, with subdomain support planned for
later.
...
1. **Ownership Verification**: All space operations verify that the
   authenticated user owns the space
2. **Reserved Slugs**: Prevents conflicts with system routes
3. **Public Profile Data**: Only public information is exposed (no sensitive
   data)
...
3. **Sync user to Payload** (or create Payload user with matching email)
4. **Create a space** via `POST /api/user/spaces`
5. **Create posts** in Payload and assign them to the space
