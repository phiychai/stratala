...
This document explains the module system configuration across the monorepo and
when to use `.js`, `.cjs`, or `.mjs` file extensions.

> **Note**: For configuration files specifically, see
> [Configuration Files](./2.config-files) for a quick reference.
...
**Problem:** Using `require()` in a `.js` file when package has
`"type": "module"`
...
3. **Config files:** Prefer explicit extensions (`.cjs`, `.mjs`) for config
   files
