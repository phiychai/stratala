...
async function assignDefaultTenant() {
  ...
  try {
    ...
    for (const user of usersResult.rows) {
      ...
      const hasTenant = Array.isArray(tenants) && tenants.some((t: any) => {
    ...
    const tenantScopedCollections = ['spaces', 'posts', 'pages', 'categories', 'tags', 'forms', 'form_fields', 'form_submissions', 'form_submission_values'];
