...
async function setupTenants() {
  ...
  try {
    ...
    for (const user of users.docs) {
      ...
      const hasDefaultTenant = tenants.some((t: any) =>
        (typeof t === 'string' ? t : t.tenant) === defaultTenant.id
      );
        ...
        if (items.docs.length > 0) {
          ...
          for (const item of items.docs) {
            try {
              ...
              console.warn(`    ⚠ Failed to update ${collectionSlug} item ${item.id}:`, (error as Error).message);
            }
          }
